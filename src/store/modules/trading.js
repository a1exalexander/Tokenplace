...
const mutations = {
	cleanAll(state) {
		for(let item in state) {
			let type = {}.toString.call(state[item]).slice(8, -1);
	...
	sort(state, [arr, type]) {
		if(state[arr].length) {
			...
			let isNumber = array.isNumeric(type);
			if (isNumber) {
...
const actions = {
	...
	toggleFavorite({ state, commit }, id) {
		let element = state.pairs.find(item => item.id == id);
		let elementIndex = state.pairs.indexOf(element);
		if(!element.star) {
			element.star = true;
