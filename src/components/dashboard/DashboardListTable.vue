  ...
  <table class="dashboard-list-table">
    <thead class="dashboard-list-table__thead">
      <tr class="dashboard-list-table__row dashboard-list-table__row--head">
        <th
          class="dashboard-list-table__cell
            dashboard-list-table__cell--head
            dashboard-list-table__cell--coin"
          @click="sort('coin')"
        >
          <span class="dashboard-trades-table__text">coin</span>
          <icon-angle-down
            class="dashboard-list-table__head-img"
            :class="{
        ...
        <th
          class="dashboard-list-table__cell
            dashboard-list-table__cell--right
            dashboard-list-table__cell--head"
          @click="sort('price')"
        >
    ...
    <tbody class="dashboard-list-table__tbody" v-if="myApi.length">
      <tr class="dashboard-list-table__row" v-for="(item, index) in wishList" :key="index">
        ...
        <td
          class="dashboard-list-table__cell
            dashboard-list-table__cell--right
            dashboard-list-table__cell--price"
        >{{ item.price }}</td>
        <td
          class="dashboard-list-table__cell
            dashboard-list-table__cell--right
            dashboard-list-table__cell--day"
        >{{ item[sortBy] }}%</td>
        <td class="dashboard-list-table__cell">
          <img src="@/assets/images/mini_graph.jpg" alt="graph">
        </td>
        <td class="dashboard-list-table__cell">
          <button class="dashboard-list-table__delete" @click="deleteCoin(index)">
...
export default {
  ...
  methods: {
    ...mapActions('dashboard', ['sortWishList']),
    ...mapMutations('dashboard', ['deleteCoin']),
    sort(type) {
