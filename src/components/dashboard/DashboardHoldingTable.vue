...
<table class="dashboard-holding-table">
	<thead class="dashboard-holding-table__thead">
		<tr class="dashboard-holding-table__row dashboard-holding-table__row--head">
			<th 
				class="dashboard-holding-table__cell dashboard-holding-table__cell--head dashboard-holding-table__cell--coin"
				@click="sort('coin')">
				<span>coin</span>
				<icon-angle-down 
			...
			<th 
				...
				<icon-angle-down 
					class='dashboard-holding-table__head-img'
					:class='{"sort-image-active": status.value.direction, "sort-active-arrow": status.value.active}'/>	
			</th>
			<th 
				class="dashboard-holding-table__cell dashboard-holding-table__cell--head"
				@click="sort('percent')">
				...
				<icon-angle-down 
					class='dashboard-holding-table__head-img'
					:class='{"sort-image-active": status.percent.direction, "sort-active-arrow": status.percent.active}'/>	
			</th>
		</tr>
	</thead>
	<transition
	...
	<tbody class="dashboard-holding-table__tbody" v-if="myApi.length">
		<tr class="dashboard-holding-table__row" v-for="(item, index) in holdings" :key='index'>
			<td class="dashboard-holding-table__cell dashboard-holding-table__cell--coin">{{ item.coin }}</td>
			<td class="dashboard-holding-table__cell">{{ item.amount }}</td>
			<td class="dashboard-holding-table__cell dashboard-holding-table__cell--price">$ {{ item.price }}</td>
			<td class="dashboard-holding-table__cell dashboard-holding-table__cell--day">{{ item[sortBy] }}%</td>
	...
	<div class="dashboard-holding-table__lock-wrapper" v-else>
		<p class="dashboard-holding-table__lock-text">Please add your exchange API to unlock Holdings</p>
		<router-link to='/exchanges' class="dashboard-holding-table__lock-button">Add API Key</router-link>
	</div>
	</transition>
</table>
</template>
