...
<section class="trading-order-book" @click.self='hideDrop'>
	<div 
		...
		@click.self='hideDrop'>
		<div 
			class="trading-order-book__head" 
			@click.self='hideDrop'>
			<h1 
				class="trading-order-book__title" 
				@click='hideDrop'>
				Order book
			</h1>
			<button 
				class="trading-order-book__button"
				@click='decimalsVisible = !decimalsVisible'>
				<p class="trading-order-book__button-text">{{ decimalSelected }}
				</p>
				...
				<transition
					...
					<div 
						class="trading-order-book__drop-list" 
						v-show='decimalsVisible'>
						<p class="trading-order-book__drop-item" 
							v-for='(item, index) in decimals' 
							:key='index' 
							@click='selectDecimal(item)'>{{ item }}</p>
					</div>
				</transition>
			</button>
		</div>
		<table 
			class="trading-order-book__table trading-order-book__table--red" 
			@click='hideDrop'>
			<thead class="trading-order-book__table-thead">
				<tr class="trading-order-book__table-row trading-order-book__table-row--head">
					<th class="trading-order-book__table-cell trading-order-book__table-cell--head trading-order-book__table-cell--price">Price</th>
					<th class="trading-order-book__table-cell trading-order-book__table-cell--head trading-order-book__table-cell--amount">amount</th>
					<th class="trading-order-book__table-cell trading-order-book__table-cell--head trading-order-book__table-cell--total">total</th>
				</tr>
			</thead>
			<tbody class="trading-order-book__table-body">
				<tr class="trading-order-book__table-row" v-for="(item, index) in orderBook" :key='index'>
					<td class="trading-order-book__table-cell trading-order-book__table-cell--price trading-order-book__table-cell--red">{{ item.price }}</td>
					<td class="trading-order-book__table-cell trading-order-book__table-cell--amount">{{ item.amount }}</td>
					<td class="trading-order-book__table-cell trading-order-book__table-cell--total">{{ item.total }}</td>
				</tr>
			</tbody>
		</table>
	</div>
	<div 
		class=" trading-order-book__wrapper trading-order-book__wrapper--green" 
		@click='hideDrop'>
		...
		<table class="trading-order-book__table trading-order-book__table--green">
			<tbody class="trading-order-book__table-body">
				<tr class="trading-order-book__table-row" v-for="(item, index) in orderBook" :key='index'>
					<td class="trading-order-book__table-cell trading-order-book__table-cell--price trading-order-book__table-cell--green">{{ item.price }}</td>
					<td class="trading-order-book__table-cell trading-order-book__table-cell--amount">{{ item.amount }}</td>
					<td class="trading-order-book__table-cell trading-order-book__table-cell--total">{{ item.total }}</td>
				</tr>
			</tbody>
