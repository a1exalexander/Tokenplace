...
<div class="trading-form-order">
	<div class="trading-form-order__toggle-box">
		<button class="trading-form-order__toggle-button" 
			:class="{'toggle-button-active': tradingForms.limit}" 
			@click="chooseForm('limit')">Limit orders
		</button>
		<button class="trading-form-order__toggle-button" 
			:class="{'toggle-button-active': tradingForms.market}" 
			@click="chooseForm('market')">Market order
		</button>
		<button class="trading-form-order__toggle-button" 
			:class="{'toggle-button-active': tradingForms.profit}" 
			@click="chooseForm('profit')">Stop-loss / Take profit
		</button>
		<button class="trading-form-order__toggle-button" 
			:class="{'toggle-button-active': tradingForms.shadow}" 
			@click="chooseForm('shadow')">Shadow order
		</button>
	</div>
...
export default {
	...
	methods: {
		chooseForm(form) {
			let forms = this.tradingForms;
			for (let el in forms) {
				el == form ? forms[el] = true: forms[el] = false;
			}
		}
	}
}
