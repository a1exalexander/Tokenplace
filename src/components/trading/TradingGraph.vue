	...
	<div class="trading-graph-nav trading-graph-nav--padding">
		<div class="trading-graph-nav__inner">
			<button class="trading-graph-drop" @click="dropListGraphsShow = !dropListGraphsShow">
				<img class="trading-graph-drop__img" 
					:src="dropListGraphs.ico" 
					:alt="dropListGraphs.name"
					v-if='getApiNamesIcons.length && dropListGraphs.ico'>
				<p class="trading-graph-drop__text">{{ getApiNamesIcons.length && dropListGraphs.name ? dropListGraphs.name: "All Exhanges" }}</p>
				<icon-angle-down class="trading-graph-drop__angle-img"
				...
				<div class="trading-graph-drop__list" v-show='dropListGraphsShow'>
					<div class="trading-graph-drop__item" 
						v-for='(item, index) in getApiNamesIcons' 
						:key='index' 
						@click='selectGraph(item)'>
						<img class="trading-graph-drop__img trading-graph-drop__img--item" 
							:src="item.ico" alt="image">
...
export default {
	...
	methods: {
		...mapMutations('trading', [
			"setGraph"
		]),
		chooseFilter(type) {
			let filters = this.timeFilter;
			for (let el in filters) {
				el == type ? filters[el] = true: filters[el] = false;
			}
		},
		selectGraph(item) {
