<template>
  <section class="two-step">
    <article class="two-step__card animated fadeIn dur08">
      <button class="two-step__button-close" @click="cancel">
        <icon-close class="two-step__close-img"/>
      </button>
      <div class="two-step__logo-wrapper">
        <icon-logo class="sign-in__logo"/>
      </div>
      <p class="two-step__heading animated fadeIn slow" v-if="success">It works!
        <br>You have set up 2-step Verification
      </p>
      <p class="two-step__heading" v-else>Two-step verification
        <br>By Google Authenticator set up
      </p>
      <p
        class="two-step__text animated delay-02 fadeIn slow"
        v-if="success"
        >If you want to turn off 2-step Verification — you can do it in your settings,
        in "Security" section
      </p>
      <form v-else>
        <ol class="two-step__list">
          <li class="two-step__item"
            >Install Google Authenticator App on your Android or IOS device
          </li>
          ...
          <li class="two-step__item">
            Write your Authentication code (2fa)
            <input
              type="text"
...
export default {
  ...
  methods: {
    ...mapMutations('settings', ['enableTwoStepVerification']),
    apply() {
      this.buttonLoading = true;
      http
        .post('https://jsonplaceholder.typicode.com/posts', this.code2fa)
        .then(() => {
  ...
  computed: {
    showTitle() {
      return this.success
        ? 'It works! You have set up 2-step Verification'
        : 'Two-step verification By Google Authenticator set up';
    },
    checkCode() {
      return !!this.code2fa;
    },
