<template>
  <main class="plans">
    ...
    <section class="plans__header">
      ...
      <p
        class="plans__text plans__text--header"
    ...
    <section class="plans__section">
      <article class="plan" v-for="plan in plans" :key="plan.id">
        ...
        <div class="plan__inner plan__inner--footer">
          <button
            class="plan__button animated dur06 fadeIn"
            v-if="!plan.status"
            @click="buyPlan(plan)"
          >Buy now</button>
          <button
            class="plan__text plan__text--button-cancel animated delay-03 dur04 fadeInUp"
            v-if="plan.status"
            @click="cancelPlan(plan.id)"
          >Cancel subscription</button>
          <p class="plan__expiry animated delay-04 dur04 fadeInUp" v-if="plan.status">
            Date of Expiry:
...
export default {
  ...
  computed: {
    ...mapState('settings', ['plans']),
  },
  methods: {
    ...mapActions('settings', ['getPlans']),
    ...mapMutations('settings', ['cancelPlan']),
    hidePopup() {
      this.popup = false;
    },
    buyPlan(param) {
