    ...
    <section class="support__box">
      <section class="support__header">
        ...
        <button class="support__button-empty">
          <p
            ...
            @click="newSupportPopup = true"
          >Send new request to support</p>
      ...
      <ul class="support__row">
        ...
        <li
          ...
          @click="$sortDates('lastDate')"
        >last massage date
          <icon-angle-down
            class="support__head-img"
            :class="{
              'sort-image-active': statusSort.lastDate.direction,
              'sort-active-arrow': statusSort.lastDate.active
            }"
          />
        </li>
      ...
      <section class="support__element" v-for="(item, index) in support" :key="item.id">
        <ul class="support__row">
          ...
          <li class="
            support__cell
            support__cell--right support__cell--date">{{ item.lastDate }}</li>
          <li
            class="support__cell support__cell--status"
            :class="statusMessageClass[item.status]"
          >{{ statusMessageText[item.status] }}</li>
          <li class="support__cell support__cell--right support__cell--details">
            <button class="support__button-empty" @click="toggleBox(index)">
        ...
        >
          <section class="support__message-box" v-if="item.box">
            <button class="support__button-close" @click="item.box = false">
            ...
            >
              <p
                ...
                :class="{'support__message-time--incoming': message.incoming}"
              >{{ message.time }}</p>
              <section
              ...
              >
                <p
                  ...
                  :class="{'support__message-text--incoming': message.incoming}"
                >{{ message.text }}</p>
              </section>
            </article>
...
export default {
  ...
  methods: {
    ...mapActions('support', ['downloadAll']),
    ...mapMutations('support', ['toggleBox', 'newMessage', 'sortDates']),
