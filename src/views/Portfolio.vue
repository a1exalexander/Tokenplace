    ...
    <div class="portfolio__box" @click.self="hideDrop">
      <div class="portfolio__head" @click.self="hideDrop">
        ...
        <div class="portfolio__head-inner" @click.self="hideDrop">
          ...
          >
            <p
              class="portfolio__filter-text"
            >{{ filterExchanges.selected? filterExchanges.selected: "All Exchanges" }}</p>
            ...
            >
              <div class="portfolio__drop-list" v-show="filterExchanges.dropList">
                <p
                  class="portfolio__drop-item"
                  v-for="(item, index) in getApiNames"
                  :key="index"
                  @click="selectFilterByName(item.name)"
                >{{ item.name }}</p>
              </div>
            </transition>
          </button>
      ...
      <ul class="portfolio__table-row portfolio__table-row--head" @click="hideDrop">
        <li class="portfolio__table-cell portfolio__table-cell--head" @click="sort('coin')">
          <p class="portfolio__table-text">coin</p>
          <icon-angle-down
            class="portfolio__angle-img"
            :class="{
              'sort-image-active': statusSort.coin.direction,
              'sort-active-arrow': statusSort.coin.active
            }"
          />
        </li>
        <li
        ...
        >
          ...
          <icon-angle-down
            class="portfolio__angle-img"
            :class="{
              'sort-image-active': statusSort.amount.direction,
              'sort-active-arrow': statusSort.amount.active
            }"
          />
        </li>
        <li class="portfolio__table-cell portfolio__table-cell--head" @click="sort('exchanges')">
          <p class="portfolio__table-text">exchanges</p>
          <icon-angle-down
            class="portfolio__angle-img"
            :class="{
              'sort-image-active': statusSort.exchanges.direction,
              'sort-active-arrow': statusSort.exchanges.active
            }"
          />
        </li>
        <li class="portfolio__table-cell portfolio__table-cell--head" @click="sort('price')">
          <p class="portfolio__table-text">price, usd</p>
          <icon-angle-down
            class="portfolio__angle-img"
            :class="{
              'sort-image-active': statusSort.price.direction,
              'sort-active-arrow': statusSort.price.active
            }"
          />
        </li>
        <li
        ...
        >
          ...
          <icon-angle-down
            class="portfolio__angle-img"
            :class="{
              'sort-image-active': statusSort.day.direction,
              'sort-active-arrow': statusSort.day.active
            }"
          />
        </li>
        <li
        ...
        >
          ...
          <icon-angle-down
            class="portfolio__angle-img"
            :class="{
              'sort-image-active': statusSort.value.direction,
              'sort-active-arrow': statusSort.value.active,
            }"
          />
        </li>
        <li
        ...
        >
          ...
          <icon-angle-down
            class="portfolio__angle-img"
            :class="{
              'sort-image-active': statusSort.percent.direction,
              'sort-active-arrow': statusSort.percent.active,
            }"
          />
        </li>
      </ul>
        ...
        <div class="portfolio__elements" v-if="myApi.length">
          <article class="portfolio__element" v-for="(item, index) in getPortfolio" :key="item.id">
            <ul class="portfolio__table-row">
              ...
              <li
                class="portfolio__table-cell
                      portfolio__table-cell--end
                      portfolio__table-cell--percent"
              >{{ item.percent }}%</li>
              <li
              ...
              >
                <button
                  class="portfolio__button portfolio__button--deposit"
              ...
              <li class="portfolio__table-cell portfolio__table-cell--end">
                <button
                  class="portfolio__button portfolio__button--withdraw"
            ...
            >
              <section class="deposit-window" v-if="item.deposit" key="deposit">
                <button class="deposit-window__close" @click="toggleDeposit(index)">
                ...
                <div class="deposit-window__inner">
                  ...
                  <button class="deposit-window__button" @click="copy">Copy</button>
                </div>
                <p
                  class="deposit-window__information"
                >IMPORTANT: Send only BTC to this deposit address</p>
                <div class="deposit-window__line"></div>
              </section>
              <section class="withdraw-window" v-else-if="item.withdraw" key="withdraw">
                <button class="withdraw-window__close" @click="toggleWithdraw(index)">
                  <icon-close class="withdraw-window__close-icon"/>
                </button>
                <div class="withdraw-window__row">
                  <p
                    class="withdraw-window__info withdraw-window__info--margin-right"
                  >Your have 0.000400906 BTC available for withdrawal</p>
                ...
                <div class="withdraw-window__inner">
                  ...
                  <label class="withdraw-window__label">
                    ...
                    <input
                      ...
                      :class="{
                        'input-error': !checkAmount && withdraw.amount,
                        'input-accepted': checkAmount
                      }"
                      v-model.number="withdraw.amount"
                    >
                  </label>
                  <button
                    class="withdraw-window__button"
                    :class="{'button-loading': item.loading}"
                    @click="toggleLoading(item.id)"
                  >Withdraw</button>
                  <p
                    class="withdraw-window__text withdraw-window__text--transaction"
                  >Transaction Fee:</p>
...
export default {
  ...
  computed: {
    ...
    getApiNames() {
      const { selected } = this.filterExchanges;
      const newArray = this.getApiNamesIcons.concat({ name: 'All Exchanges' });
      return newArray.filter(el => el.name !== selected);
    },
    getPortfolio() {
      const { selected } = this.filterExchanges;
      let array = this.portfolio;
      if (selected === 'All Exchanges') {
        return this.searchInArray(array);
      }
      const re = new RegExp(selected, 'gi');
      array = array.filter(el => el.exchanges.match(re));
      return this.searchInArray(array);
    },
    checkAmount() {
      return this.withdraw.amount
        ? this.validAmount(this.withdraw.amount)
        : false;
    },
  },
  methods: {
    ...
    searchInArray(array) {
      ...
      if (chars && this.validChars(chars)) {
        ...
        return array.filter(el => el.coin.match(re));
      }
      return array;
    },
    validChars(chars) {
    ...
    ...mapMutations('portfolio', ['toggleLoading']),
    selectFilterByName(name) {
      this.filterExchanges.selected = name;
    },
    copy() {
      const link = document.querySelector('#deposit-address');
