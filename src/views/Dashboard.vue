  ...
  <main class="dashboard" @click.self="hideDrop">
    <section class="dashboard__box dashboard__box--holding" @click.self="hideDrop">
        ...
        <div class="dashboard__header-inner" @click.self="hideDrop">
          <div class="dashboard__day-wrapper" @click.self="hideDrop">
            ...
            <button class="dashboard__sort-button" @click="toggleSort('holdingsTime')">
              ...
              <icon-angle-down
                class="dashboard__sort-button-img"
              ...
              >
                <div class="dashboard__drop-list" v-show="dropVisible.holdingsTime">
                  <p
                    class="dashboard__drop-item"
                    v-for="(item, index) in timeList"
                    :key="index"
                    @click="selectHoldingsSortTime(item)"
                  >{{ item }}</p>
                </div>
              </transition>
            </button>
      ...
      <div class="dashboard__balance-wrapper" @click="hideDrop">
        ...
        <p class="dashboard__balance">$ {{ balance }}</p>
      </div>
    </section>
              ...
              >
                <div class="dashboard__drop-list" v-show="dropVisible.wishListTime">
                  <p
                    ...
                    v-for="(item, index) in timeList"
                    :key="index"
                    @click="selectWishListSortTime(item)"
                  >{{ item }}</p>
                </div>
              </transition>
            </button>
            ...
            >
              ...
              >
                ...
                <div class="dashboard__drop-list-inner">
                  <div
                    class="dashboard__drop-item dashboard__drop-item--addcoin"
                    v-for="(item, index) in searchCoins"
                    :key="index"
                    @click="_pushData(item)"
                  >
                    <p class="dashboard__drop-item-name">{{ item }}</p>
                    <icon-plus
                      class="dashboard__sort-button-plus dashboard__sort-button-plus--droplist"
                    />
                  </div>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    ...mapMutations('dashboard', ['pushData']),
    _pushData(name) {
      const data = {
        coin: name,
        exchange: 'LIVECOIN',
  ...
  computed: {
    ...mapState('dashboard', ['balance', 'lastActivity', 'wishListCoins']),
    searchCoins() {
      if (this.searchChars) {
        const re = new RegExp(this.searchChars, 'gi');
        return this.wishListCoins.filter(el => el.match(re));
      }
