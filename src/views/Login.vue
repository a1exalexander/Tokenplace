  ...
  <div class="login">
    <transition
      ...
      <sign-in
        ...
        key="signIn"
      />
      <sign-up class="login__box" @toSignIn="toSignIn" v-else-if="visible.signUp" key="signUp"/>
      <two-step-login
        class="login__box"
        v-else-if="visible.twoStepVerification"
        key="twoStepVerification"
      />
      <reset-password class="login__box" @toSignIn="toSignIn" v-else key="resetPassword"/>
    </transition>
  </div>
</template>
<script>
